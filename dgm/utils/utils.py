...
def load_data(data_name, path, labels = None, conv = False, seed = 0):
    ...
    if data_name == 'omni':
        from dgm.models.import_data_omni import load_data_omni
        data_train, data_test, labels_train, labels_test = \
            load_data_omni(path, labels, conv, seed)
            
    if data_name == 'cifar10':
        from dgm.models.import_data_cifar10 import load_data_cifar10
        data_train, data_test, labels_train, labels_test = \
...
def init_variables(sess, old_var_list = set([])):
    all_var_list = set(tf.global_variables())
    init = tf.variables_initializer(var_list = all_var_list - old_var_list)
    sess.run(init)
    return all_var_list
