...
from dgm.utils.utils import init_variables, save_params, load_params
...
def main(data_name, method, dimZ, dimH, n_channel, batch_size, K_mc, checkpoint, lbd):
    ...
    if method == 'onlinevi':
        ...
        from dgm.alg.onlinevi import construct_optimizer, init_shared_prior, \
    ...
    if method in ['ewc', 'noreg', 'laplace', 'si']:
        ...
        if method in ['ewc', 'noreg']:
            from dgm.alg.vae_ewc import construct_optimizer, lowerbound
        if method == 'ewc': from dgm.alg.vae_ewc import update_ewc_loss, compute_fisher
        if method == 'laplace':
            from dgm.alg.vae_laplace import construct_optimizer, lowerbound
            from dgm.alg.vae_laplace import update_laplace_loss, compute_fisher, init_fisher_accum
        if method == 'si':
            from dgm.alg.vae_si import construct_optimizer, lowerbound, update_si_reg
